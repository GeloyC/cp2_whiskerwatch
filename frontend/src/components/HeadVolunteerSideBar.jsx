...
const HeadVolunteerSideBar = ({className}) => {
  ...
  const sideItemStyle = 'flex flex-col items-start w-full h-auto pt-2 pb-2 cursor-pointer bg-white border-2 border-white';
  const sideItemStyleCurrent = 'flex flex-col items-start w-full h-auto pt-2 pb-2 cursor-pointer border-2 border-white text-[#DC8801] bg-[#FDF5D8]';
  const sideItemDownCurrent = 'flex flex-col items-start w-full h-auto pt-2 pb-2 cursor-pointer border-2 border-white';
    ...
    <div className={`relative flex flex-col w-auto h-screen bg-[#FFF] z-50 ${className}`}>
      ...
      <div className={`flex items-center justify-center gap-4  w-auto h-auto box-border ${sidebarShow ? 'pl-3 pr-12' : ''} pt-4 pb-4 cursor-pointer bg-white border-b-2 border-b-[#DC8801]`}>
        ...
        <div className={sidebarShow ? 'leading-tight' : 'hidden'}>
          <label className='font-bold text-[18px]'> HEAD </label>
          <label className='font-bold text-[18px]'> VOLUNTEER </label>
        </div>
      ...
      <div className={`flex flex-col ${!sidebarShow ? 'items-center' : ''} justify-between h-full p-1`}>
          ...
          <div className={location.pathname === "/feedingvolunteers" || location.pathname === "/feedingapplications" ? sideItemDownCurrent : sideItemStyle}>
            <div className='flex flex-row items-center justify-between w-full active:text-[#B5C04A]'>
              ...
              <button onClick={() => {toggleDropdown('feeding');}} className={sidebarShow ? 'flex justify-center items-center w-[30px] h-auto p-[8px] rounded-[25px] hover:bg-[#FDF5D8] active:bg-[#FFF]' : 'hidden'}>
                <img src="/src/assets/icons/down-arrow-orange.png" alt="orange arrow" className={!dropdown.feeding & sidebarShow ? 'rotate-0' : '-rotate-90'} />
              </button>
            </div>
        ...
        <div className={`relative flex flex-row items-center ${sidebarShow ? 'justify-between pr-2' : 'justify-center w-fit pr-1'} p-1  gap-3 text-[#000] font-bold bg-[#DC8801] rounded-[50px]`}>
          ...
          {sidebarShow && (
            <button ref={menuRef} onClick={toggleVisibilityProfile} className='flex justify-center items-center w-[30px] h-auto p-2 bg-[#f0f2c8] hover:bg-[#E3E697] active:bg-[#f0f2c8] rounded-[15px]'>
