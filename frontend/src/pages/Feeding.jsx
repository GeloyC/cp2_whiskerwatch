...
const Feeding = () => {
  ...
  const [applicationStatus, setApplicationStatus] = useState('None'); // New state for application status
  ...
  const generateFeedingForm = async () => {
    ...
    try {
      ...
      const filename = `feeding-form-${userData.firstname}-${userData.lastname}.png`;
  ...
  const fetchFeedingDate = async () => {
    ...
    try {
      ...
      const { feeding_date, status, has_report } = response.data;

      setFeedingDate(feeding_date || '');
    ...
    <div className="flex flex-col min-h-screen">
      <CatBot
        message={
          applicationStatus === 'Pending'
            ? `Hi ${user?.firstname}, your application is pending approval. We'll notify you soon!`
            : !feedingDate
            ? `You can change a cat's day — and maybe their life. As a feeding volunteer, you'll help provide comfort, care, and love to cats waiting for their forever homes. Join us and make your kindness count!`
      ...
      <div className="flex flex-col flex-1 w-full">
        <div className="flex flex-col justify-center flex-1">
          <div
            className={`relative flex flex-col items-center w-auto rounded-[25px] ${
          ...
          >
            {user?.role !== 'head_volunteer' ? (
              ...
              <div className='flex flex-col items-center justify-center p-7 bg-[#FFF] w-full h-full xl:h-fit lg:h-fit xl:w-[875px] lg:w-[875px] gap-4 rounded-[10px] '>
