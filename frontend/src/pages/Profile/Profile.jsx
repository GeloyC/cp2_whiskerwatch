...
const UserCertificates = () => {
    ...
    useEffect(() => {
        ...
        try {
            const response = await axios.get(`${url}/user/certificates`, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
    ...
    // Fetch user adoption applications
    useEffect(() => {
        const fetchApplications = async () => {
        try {
    ...
    // Handle opening and closing the certificate preview
    const handleViewCert = (app) => {
        setSelectedCertificate(app);
    };
    ...
    // Convert HTML to image (download)
    const handleDownloadCert = async () => {
        const node = document.getElementById('certificate-block');
        if (!node) return;

        try {
        const dataUrl = await toPng(node);
        const link = document.createElement('a');
        link.download = `Adoption_Certificate_${selectedCertificate.cat_name}_${profile.firstname}_${profile.lastname}.png`;
        link.href = dataUrl;
        link.click();
        } catch (error) {
    ...
    return (
        <div className='flex flex-col items-center p-6 w-full h-full bg-[#f8f8f8]'>
        <h2 className='text-3xl font-bold text-[#2F2F2F] mb-4'>Your Certificates</h2>

        <div className='flex flex-col w-full max-w-[800px] gap-4'>

            {/* 🔹 Certificates already stored in database */}
            ...
            userCertificates.map((cert, index) => (
                ...
                <a
                    ...
                    target='_blank'
                    rel='noopener noreferrer'
                    className='flex items-center justify-between gap-3 p-3 pl-4 pr-4 bg-[#E3E697] text-[#2F2F2F] rounded-[10px] hover:underline border-dashed border-2 border-[#99A339]'
                >
                    View Certificate #{index + 1}
                    <div className='w-[25px] h-auto'>
                    <img src='/assets/icons/document-black.png' alt='doc' />
                    </div>
                </a>
                )
            ))}

            {/* 🔸 Automatically generate for accepted adoptions */}
            {applications.length > 0 &&
            applications.map((app) =>
                app.status === 'Accepted' ? (
                <button
                    key={app.application_id}
                    onClick={() => handleViewCert(app)}
                    className='flex items-center justify-between gap-3 p-3 pl-4 pr-4 bg-[#FDF5D8] text-[#2F2F2F] rounded-[10px] hover:underline border-dashed border-2 border-[#B5C04A]'
                >
                    View Adoption Certificate for {app.cat_name}
                    <div className='w-[25px] h-auto'>
                    <img src='/assets/icons/document-black.png' alt='doc' />
                    </div>
                </button>
                ...
                <div
                    key={app.application_id}
                    className='flex flex-col gap-2 p-5 rounded-[10px] border-dashed border-2 border-[#99A339] bg-[#FFF]'
                >
                    <span className='flex flex-row w-full justify-between'>
                    <label className='font-bold text-lg text-[#2F2F2F]'>
                        You submitted an adoption application
        ...
        {/* 📜 Modal for generated certificate */}
        {selectedCertificate && (
            <div className='fixed inset-0 flex flex-col items-center justify-center bg-black/50 backdrop-blur-sm z-50 p-10'>
            <div
                id='certificate-block'
                className='relative flex flex-col items-center bg-white bg-[url(/assets/AdoptionCertificate/Signed_Adoption_Certificate.png)] bg-cover bg-center w-[1020px] h-[650px] rounded-[20px] shadow-xl'
            >
                <label className='absolute top-[380px] text-4xl font-bold text-[#2F2F2F]'>
                {selectedCertificate.cat_name}
                </label>
                <label className='absolute top-[460px] right-[300px] text-2xl font-bold text-[#2F2F2F]'>
                {profile.firstname} {profile.lastname}
                </label>
                <label className='absolute top-[500px] right-[450px] text-xl font-bold text-[#2F2F2F]'>
                {new Date(selectedCertificate.application_date).toLocaleDateString()}
                </label>
            </div>
