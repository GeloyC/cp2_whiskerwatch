...
const AdoptersList = () => {
  ...
  // Upload certificate (using html-to-image)
  const handleUploadCert = async () => {
    const certificateNode = document.getElementById('certificate-block');
    if (!certificateNode || !selectedAdoptee) return;
        ...
        <div className='relative flex flex-col items-center xl:p-10 lg:p-10 h-screen w-full gap-5 mx-auto'>
          ...
          <div className='xl:hidden lg:hidden flex flex-col justify-center items-center h-screen w-full gap-3 rounded-[15px]'>
            ...
            <label className='text-[#8f8f8f] text-center'>
              You can access the page on larger screens such as desktop/laptop.
            </label>
          </div>
            ...
            <tbody className='flex flex-col w-full overflow-y-scroll scrollbar-thin h-[550px] gap-1'>
              {adopters.map((adoptee) => (
                <tr
                  ...
                  className='grid grid-cols-6 w-full place-items-center justify-items-start bg-[#FFF] p-2 rounded-[15px] text-[#2F2F2F] border-b-1 border-b-[#595959]'
                >
                  <td>{adoptee.adoption_id}</td>
                  <td>{adoptee.adopter}</td>
                  <td>{adoptee.cat_name}</td>
                  ...
                  <td className='flex items-center justify-start gap-2'>
                    <button
                      onClick={() => handleViewCert(adoptee)}
                    ...
                    {selectedAdoptee && selectedAdoptee.adoption_id === adoptee.adoption_id && (
                      <div className='absolute inset-0 flex flex-col items-center justify-center p-10 bg-black bg-opacity-60 z-50'>
                        <div
                          id='certificate-block'
                          className='relative flex flex-col items-center bg-[#FFF] bg-[url(/assets/AdoptionCertificate/Signed_Adoption_Certificate.png)] bg-cover bg-center w-[1020px] h-[650px] shadow-lg rounded-[20px]'
                        >
                          ...
                          {/* If a certificate exists, show it inline */}
                          {certificateUrl && (
                            <img
                              src={certificateUrl}
                              alt='Certificate'
                              className='absolute inset-0 w-full h-full object-contain rounded-[20px]'
                            />
                          )}
                        </div>
